...
const slice = createSlice({
    name: 'input',
    initialState: { value: '', display: '', history: [] },
    reducers: {
        setValue: (state, action) => {
            const value = action.payload;
            const lastHistory = state.history.at(-1) || '';
            const isLastOperator = operator.includes(lastHistory);
            const isNewOperator = operator.includes(value);
            ...
            if (isNewOperator) {
                ...
                state.display = lastHistory + value;
            } else {
                state.history.push(value);
                state.display = value;
            }
